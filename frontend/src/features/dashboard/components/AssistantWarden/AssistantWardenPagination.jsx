...
export default function AssistantWardenPagination({
    currentPage,
    totalPages,
    totalAssistantWardens,
    itemsPerPage,
    setCurrentPage
}) {
    return (
        <div className="hidden md:flex flex-row p-3 sm:p-4 bg-white border border-gray-50 items-center justify-between text-[10px] sm:text-xs font-medium text-gray-500 rounded-b-xl shadow-sm shrink-0 mt-auto">
            <div>
                ...
                {totalAssistantWardens === 0 ? 0 : (currentPage - 1) * itemsPerPage + 1}
                <span className="hidden sm:inline"> to </span>
                <span className="sm:hidden">-</span>
                {Math.min(currentPage * itemsPerPage, totalAssistantWardens)} of {totalAssistantWardens}
