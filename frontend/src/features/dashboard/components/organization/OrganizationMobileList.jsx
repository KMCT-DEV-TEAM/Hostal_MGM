        ...
        <div className="md:hidden flex flex-col gap-4 mt-4 md:mt-0 flex-1 overflow-y-auto pb-4 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
            ...
            {loading ? (
                ...
                orgs.map((o) => {
                    ...
                    return (
                    <div key={o._id} className={`bg-white p-4 rounded-xl shadow-sm flex flex-col relative border ${isSelected && !isAdmin ? 'border-[#0A437A] bg-blue-50/20' : 'border-transparent'}`}>
                        {!isAdmin && (
