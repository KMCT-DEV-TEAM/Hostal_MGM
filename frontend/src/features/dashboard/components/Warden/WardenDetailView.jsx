...
const WardenDetailView = ({ selectedWardenDetail, setView, openChangeEmailModal }) => {
            ...
            if (hostelId && hostelId !== 'Not Assigned') {
                const fetchHostel = async () => {
                    ...
                    try {
                        ...
                        console.error('Failed to fetch hostel details for warden:', err);
                    } finally {
    ...
    return (
        <Modal
            ...
            maxWidth="max-w-5xl"
            title={selectedWardenDetail?.name || 'Warden'}
            subtitle="Warden Details"
            icon={<User size={24} />}
        >
            <div className="grid grid-cols-1 md:grid-cols-12 gap-5">
                ...
                <div className="md:col-span-7 space-y-4 md:space-y-6">
                    ...
                    <div className="bg-white p-4 md:p-6 rounded-xl border border-gray-200 shadow-sm">
                        ...
                        <div className="space-y-1">
                            ...
                            <InfoRow label={<><Phone className="w-4 h-4 text-gray-400" /> Phone</>}>{selectedWardenDetail?.phone ? `+91 ${selectedWardenDetail.phone}` : 'N/A'}</InfoRow>
                            <InfoRow label={<><Mail className="w-4 h-4 text-gray-400" /> Email</>}>
                                <span className="flex items-center justify-between w-full">
                                    <span className="flex-1 break-all pr-2">{selectedWardenDetail?.email || 'N/A'}</span>
                                    <button
                                        onClick={() => openChangeEmailModal && openChangeEmailModal(selectedWardenDetail)}
                ...
                <div className="md:col-span-5 bg-white p-4 sm:p-6 rounded-xl border border-gray-200 shadow-sm h-fit">
                    <h3 className="text-sm font-semibold text-[#0A437A] mb-3 md:mb-4">Warden Summary</h3>
                    <div className="space-y-1">
                        <InfoRow label={<><User className="w-4 h-4 text-gray-400" /> Name</>}>{selectedWardenDetail?.name}</InfoRow>
                        <InfoRow label={<><Building2 className="w-4 h-4 text-gray-400" /> Hostel</>}>{selectedWardenDetail?.hostel?.name || selectedWardenDetail?.hostel || 'N/A'}</InfoRow>
                        <InfoRow label={<><ToggleRight className="w-4 h-4 text-gray-400" /> Status</>}>
...
export default WardenDetailView;