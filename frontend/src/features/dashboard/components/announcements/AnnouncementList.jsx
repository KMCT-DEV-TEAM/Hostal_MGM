...
export const AnnouncementCard = ({ announcement, className = "", onClick }) => {
    const { Icon, bg, text } = getIconAndStyle(announcement.title);
    return (
        <div
            onClick={onClick}
            className={`bg-white rounded-[20px] md:border md:border-gray-100 shadow-[0_2px_12px_rgb(0,0,0,0.03)] md:shadow-sm md:hover:shadow-md transition-all p-[18px] flex gap-[16px] h-full ${onClick ? 'cursor-pointer' : ''} ${className}`}
        >
...
const AnnouncementList = ({ announcements, onAnnouncementClick, loading, loadingMore, observerRef, isStudentOrParent }) => {
    ...
    return (
        <div className="md:py-4 md:p-4 grid grid-cols-1 md:grid-cols-2 gap-4">
